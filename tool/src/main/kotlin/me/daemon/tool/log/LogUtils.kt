...
fun v(tag: Any, vararg msg: Any?) {
    val l = logger
    if (!filter.filter(l, Level.VERBOSE, tag, null, msg)) return
...
fun v(tag: Any, tr: Throwable, vararg msg: Any?) {
    val l = logger
    if (!filter.filter(l, Level.VERBOSE, tag, tr, msg)) return
...
fun d(tag: Any, vararg msg: Any?) {
    val l = logger
    if (!filter.filter(l, Level.DEBUG, tag, null, msg)) return
...
fun d(tag: Any, tr: Throwable, vararg msg: Any?) {
    val l = logger
    if (!filter.filter(l, Level.DEBUG, tag, tr, msg)) return
...
fun i(tag: Any, vararg msg: Any?) {
    val l = logger
    if (!filter.filter(l, Level.INFO, tag, null, msg)) return
...
fun i(tag: Any, tr: Throwable, vararg msg: Any?) {
    val l = logger
    if (!filter.filter(l, Level.INFO, tag, tr, msg)) return
...
fun w(tag: Any, vararg msg: Any?) {
    val l = logger
    if (!filter.filter(l, Level.WARN, tag, null, msg)) return
...
fun w(tag: Any, tr: Throwable, vararg msg: Any?) {
    val l = logger
    if (!filter.filter(l, Level.WARN, tag, tr, msg)) return
...
fun e(tag: Any, vararg msg: Any?) {
    val l = logger
    if (!filter.filter(l, Level.ERROR, tag, null, msg)) return
...
fun e(tag: Any, tr: Throwable, vararg msg: Any?) {
    val l = logger
    if (!filter.filter(l, Level.ERROR, tag, tr, msg)) return
